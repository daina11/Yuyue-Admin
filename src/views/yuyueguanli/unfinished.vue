    ...
    >
      ...
      <el-table-column align="center" label="操作">
        <template slot-scope="{ row }">
          ...
          <el-button type="success" size="mini" @click="suc(row)">
            到店
          </el-button>
...
export default {
  ...
  methods: {
    suc(e) {
      ...
      succeeorder({id:e.id})
        .then((re) => {
          ...
          shopfindbystutas(this.shopid, this.page)
            .then((re) => {
    ...
    cur(e) {
      this.page = e - 1;
      shopfindbystutas(this.shopid, e - 1)
        .then((re) => {
          this.tableData = re.data.content;
          this.totalelement = re.data.totalElements;
        })
  ...
  created() {
    getshopbyuid(this.userid)
      .then((res) => {
        ...
        shopfindbystutas(res.data.id, this.page)
          .then((re) => {
            this.tableData = re.data.content;
            this.totalelement = re.data.totalElements;
          })
